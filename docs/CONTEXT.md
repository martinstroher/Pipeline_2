...
The ablation, statistical analyses, expert workbooks, rehearsal, and usability-pilot documentation are isolated under `evaluation_study/`. They read frozen production artifacts but are not imported or dispatched by `pipeline.py`. The vNext workbook translates opaque ontology labels through `config/display_text.yaml`, shows a short task banner on every expert-facing sheet, and uses existing frozen NLDs only to clarify terms flagged as ambiguous in the pilot. It gives every expert the same sampled items, captures completion time, and analyzes simplified definition/relation judgments plus plain-language meaning preservation for sampled exclusions and demotions while retaining both Taxonomy judgments. It makes no runtime LLM call. See `evaluation_study/README.md`.
