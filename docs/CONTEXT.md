...
- **Tech**: gpt-5.4. Focused calls run in barriers: taxonomy correctness → class worthiness → local/global term reconciliation → facet/frame audit → corpus-only frame completion + standard NLD/targeted relations → relation correctness → relation scope. Categories run in parallel within each barrier.
...
- **Focused lateral stages:** `critic_class_worthiness.txt` decides primitive/defined/demote/drop; `critic_taxonomy_dedup.txt` reconciles local survivors plus BGE-M3-shortlisted cross-category NLD pairs; `critic_facet_frames.txt` receives candidate-parent NLD/ancestry/category/metatype context; `critic_frame_completion.txt` accepts only corpus-attested missing kinds, which then use the standard NLD and Step-6b relation workflows; relation critics receive rich subject/filler contexts.
...
- **Facet/frame audit:** runs after global reconciliation; it can REPARENT to existing classes, collapse useless singleton intermediates, diagnose same/mixed-axis frames, and propose completion candidates/disjointness. Completion terms are added only after distinct-document corpus attestation and evidence verification, then receive a standard RAG-grounded NLD and targeted Step-6b relations before relation criticism.
