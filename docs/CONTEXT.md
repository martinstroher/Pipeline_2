...
- The ontology header declares the configured ontology IRI, `owl:versionInfo`, `owl:versionIRI`, and every configured `owl:imports` value.
- A versioned correction manifest can drop an incompatible restriction, reparent a class, or replace the genus inside a defined-class intersection. The mechanism is domain-neutral; subjects and geological justifications remain in the active domain manifest.
- Every emitted graph is canonicalized for a complete one-row-per-triple provenance CSV. Replay compares both graph isomorphism and the canonical provenance triple list.
...
- **Robustness guards:** Unicode labels are normalized and transliterated before CamelCase IRI minting. Public comments never include critic directives. Every graph correction must match exactly one target axiom or emission fails.
...
### `src/modules/emit/verifier.py` — Step 7b: Ontology Verification
...
The ablation, statistical analyses, expert workbooks, rehearsal, and usability-pilot documentation are isolated under `evaluation_study/`. They read frozen production artifacts but are not imported or dispatched by `pipeline.py`. The vNext workbook translates opaque ontology labels through `config/display_text.yaml`, shows a short task banner on every expert-facing sheet, and requires one approved condition-independent reference definition for every Category Correct and Meaning Preservation term. It gives every expert the same sampled items, captures completion time, and analyzes simplified definition/relation judgments plus plain-language meaning preservation for sampled exclusions and demotions while retaining both Taxonomy judgments. It makes no runtime LLM call and hashes the approved definition artifact in the workbook manifest. See `evaluation_study/README.md`.
