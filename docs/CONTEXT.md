...
The ablation, statistical analyses, expert workbooks, rehearsal, and usability-pilot documentation are isolated under `evaluation_study/`. They read frozen production artifacts but are not imported or dispatched by `pipeline.py`. The vNext workbook translates opaque ontology labels through `config/display_text.yaml`, explicitly distinguishes geological-age properties from named time intervals, renders complete non-circular definition sentences, and presents each task with a four-row plain-language instruction panel. Friendly visible sheet and column names are translated back to stable internal analysis keys. One approved condition-independent definition is required for every Category Review and Removed or Rewritten Terms item. Representation uses 100 terms stratified by Condition-A tier and frequency; Category Review separately samples 60 A-vs-baseline disagreements across overlap, tier, and frequency, with at least 25 terms per contrast. Every expert receives the same sampled items. Removed or Rewritten Terms records geological meaning separately from suitability for the main Pre-Salt model while retaining its conditional preferred outcome and both Taxonomy judgments. Layer 2 reports disagreement-only contrasts with bootstrap intervals and Holm correction, exploratory Holm-corrected Spearman correlations, raw agreement, Fleiss kappa, Gwet AC1/AC2 sensitivity, and item consensus. It makes no runtime LLM call and hashes the approved definition artifact in the workbook manifest. See `evaluation_study/README.md`.
