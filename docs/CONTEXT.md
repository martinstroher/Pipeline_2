...
- **Note (model change, 2026-07 — removable):** The threshold was previously freq≥7, chosen on the earlier *Gemini* extraction, which yielded 976 / 614 / 368 terms at freq≥5 / 7 / 10. Migrating to gpt-5.4 produced a leaner, more consolidated extraction (~43% as many terms at every threshold), so freq≥7 now yields only 265 terms. The threshold was re-derived on the new distribution and lowered to freq≥5 (407 terms) to preserve ontology coverage and Layer-2 expert-evaluation headroom (100 stratified Representation terms plus a separate 60-term disagreement-enriched Category sample). Remove this note once the change is settled in the thesis narrative.
...
- **Deterministic ancestry repair:** children of a dropped parent follow its survivor/new parent or nearest surviving ancestor; category-root fallback is last resort. A survivor alias that points back to the child is skipped, and case-insensitive upper-class identity rows suppress their self-parent edge.
- **Single-axis IS-A guard:** taxonomy/facet prompts distinguish inhering qualities and measurements from the physical structures, sites, or bearers that ground them. Cross-axis meaning belongs in relations or defined classes, never a mixed quality→structure subclass chain.
- Replaces the legacy multi-pass critic (`ontology_critic`) and the deterministic relation reclassifier (`relation_reclassifier`). The work is split into focused calls so each prompt does one job:
  ...
  - **Term reconciliation — Stage 2** (`critic_taxonomy_dedup.txt`): within-category survivors are compared directly; cross-category reconciliation takes the union of each term's top `reconciliation.top_k` BGE-M3 NLD neighbors from other categories (default 3), with no numeric similarity cutoff. The NLD vectors are in-memory and separate from the paper-chunk Chroma index. The same critic decides SAME_KIND, strict subsumption, DISTINCT, or NEEDS_REVIEW; similarity never edits directly. At most one unambiguous mutation is applied per term. Competing targets remain unapplied diagnostics, and `Applied` is recalculated from the materialized final taxonomy.
...
- **Facet/frame audit:** runs after global reconciliation; it can REPARENT to existing classes, collapse useless singleton intermediates, diagnose same/mixed-axis frames, and propose completion candidates/disjointness. Completion candidates are corpus-attested diagnostics by default (`frame_completion.auto_add: false`) and do not re-expand the core. Explicit opt-in restores evidence verification, standard RAG-grounded NLD generation, and targeted Step-6b relations.
...
- Outputs additionally include `validate_evidence_bundle.csv`, `validate_demotions.csv`, `validate_term_reconciliation.csv`, `validate_subsumption_hints.csv`, `validate_frame_completion.csv`, `validate_frame_completion_relations.csv`, and `validate_lateral_coherence_summary.json` alongside the taxonomy/relation/edit/instance/defined/facet/disjointness artifacts.
...
evaluation_study/         # Standalone thesis evaluation package; see its README
  config/                 # Workbook instructions and geologist-facing display registry
  inputs/                 # Frozen ablation-A and approved-ontology handoff CSVs
...
The ablation, statistical analyses, expert workbooks, rehearsal, and usability-pilot documentation are isolated under `evaluation_study/`. They read frozen production artifacts but are not imported or dispatched by `pipeline.py`. The vNext workbook translates opaque ontology labels through `config/display_text.yaml`, explicitly distinguishes geological-age properties from named time intervals, renders complete non-circular definition sentences, and presents each task with a four-row plain-language instruction panel. Friendly visible sheet and column names are translated back to stable internal analysis keys. One approved condition-independent definition is required for every Category Review item. Representation uses 100 terms stratified by Condition-A tier and frequency; Category Review separately samples 60 A-vs-baseline disagreements across overlap, tier, and frequency, with at least 25 terms per contrast. Every expert receives the same sampled items. Forty sampled exclusions and rewrites are generated separately in `model_changes_review.xlsx` for optional qualitative review by a geologist or ontologist; they are absent from the main workbooks, timing, and three-expert statistics. Layer 2 reports disagreement-only contrasts with bootstrap intervals and Holm correction, exploratory Holm-corrected Spearman correlations, raw agreement, Fleiss kappa, Gwet AC1/AC2 sensitivity, and item consensus. It makes no runtime LLM call and hashes the approved definition artifact in the workbook manifest. See `evaluation_study/README.md`.
