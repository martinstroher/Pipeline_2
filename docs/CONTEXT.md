...
- Replaces the legacy multi-pass critic (`ontology_critic`) and the deterministic relation reclassifier (`relation_reclassifier`). The work is split into focused calls so each prompt does one job:
  ...
  - **Term reconciliation — Stage 2** (`critic_taxonomy_dedup.txt`): within-category survivors are compared directly; cross-category pairs are shortlisted by normalized labels and an in-memory BGE-M3 cosine search over NLDs (separate from the paper-chunk Chroma index). The same critic decides SAME_KIND, strict subsumption, DISTINCT, or NEEDS_REVIEW. Similarity never edits directly; applied merges redirect taxonomy and relation references.
