...
### `src/modules/validate/critic.py` — validate: Three-Stage Critic per Category
- **Tech**: Gemini 2.5 Pro. Three kinds of LLM call per category run **sequentially** (chunked per-term taxonomy → cross-term dedup → relations); the categories themselves run in parallel via `ThreadPoolExecutor` (concurrency capped by `MAX_CONCURRENT_CRITIC`, default 5).
...
- **Defined classes:** when a KEEP_AS_BEARER carry is a *realizable* (`role` / `function` / `disposition` — via `_DEFINING_BEARER_PARENTS`; `quality` carries stay primitive), `_build_defined_classes` records `Bearer, Genus, Property, Property_IRI, Filler` to `validate_defined_classes.csv`. Stage 1 may also emit `KEEP_AS_DEFINED` with a `defined_by` block (`base_class`, `property`, `filler`, `rationale`) for central cross-axis terms whose definition uses existing classes/properties. The OWL exporter turns each row into an `owl:equivalentClass` definition — `bearer ≡ genus ⊓ (<property> some <filler>)`. Malformed or unsafe definitions fall back to KEEP.
- **Lateral-coherence audit:** Stage 1 can receive `Weak heuristic observations` when `lateral_coherence.hints.enabled` is true (override with `LATERAL_HINTS_ENABLED=false`). These observations are noisy attention cues only — they never contain actions, never directly edit the ontology, and the prompt explicitly forbids using them as evidence. The taxonomy critic emits class-worthiness metadata (`class_fate`, `centrality`, `cross_axis`, `placement_rationale`, `needs_review`, `confidence`) into `validate_edits.csv` and `validate_class_fates.csv`. It may also emit `DROP_AS_OVER_SPECIFIC` for low-centrality, non-reusable adjective stacks / cross-axis phrases / property-value refinements; this is conservative and fully logged.
- **Facet/disjointness diagnostics:** Stage 2 may emit `facet_frames` and `disjointness` diagnostics to `validate_facet_frames.csv` and `validate_disjointness.csv`. These do not edit taxonomy rows. Step 7 emits domain disjointness only when `lateral_coherence.disjointness.enabled` is true, and skips emission if `require_reasoner_validation` is true but Java/HermiT is unavailable.
- **Relation scope:** Stage 3 records `Relation_Scope` (`generic | corpus_context | individual_fact`) on surviving relation rows. Step 7 emits OWL class restrictions only for generic relations when `lateral_coherence.relation_scope.emit_only_generic_relations` is true, preventing corpus-context facts from becoming universal class axioms.
...
- Output: `output/refined/validate_taxonomy.csv`, `output/refined/validate_relations.csv`, `output/refined/validate_edits.csv`, `output/refined/validate_instances.csv`, `output/refined/validate_minted_properties.csv`, `output/refined/validate_defined_classes.csv`, `output/refined/validate_class_fates.csv`, `output/refined/validate_facet_frames.csv`, `output/refined/validate_disjointness.csv`
