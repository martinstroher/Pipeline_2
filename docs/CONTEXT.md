...
- **Defined bearer classes:** when a KEEP_AS_BEARER carry is a *realizable* (`role` / `function` / `disposition` — via `_DEFINING_BEARER_PARENTS`; `quality` carries stay primitive), `_build_defined_classes` records `Bearer, Genus, Property, Property_IRI, Filler` to `validate_defined_classes.csv`. The OWL exporter turns each row into an `owl:equivalentClass` **definition** — `bearer ≡ genus ⊓ (<property> some <minted role>)` — so a role-fused term (e.g. `Carbonate Reservoir`) becomes a *defined* class ("a sedimentary rock that plays the reservoir role") instead of a primitive rigid kind. This is the OntoClean fix for a non-rigid role baked into a class name; the class label and IRI are unchanged (traceability preserved). The genus is the bearer's final taxonomy parent (after any REPARENT).
- **Lateral-coherence diagnostics (first slice):** Stage 1 can receive `Weak heuristic observations` when `lateral_coherence.hints.enabled` is true (override with `LATERAL_HINTS_ENABLED=false`). These observations are noisy attention cues only — they never contain actions, never directly edit the ontology, and the prompt explicitly forbids using them as evidence. The taxonomy critic also emits class-worthiness metadata (`class_fate`, `centrality`, `cross_axis`, `placement_rationale`, `needs_review`, `confidence`) into `validate_edits.csv` and the derived `validate_class_fates.csv`. In this slice, those fields are diagnostic; supported ontology-changing verdicts remain the existing taxonomy actions.
...
- Output: `output/refined/validate_taxonomy.csv`, `output/refined/validate_relations.csv`, `output/refined/validate_edits.csv`, `output/refined/validate_instances.csv`, `output/refined/validate_minted_properties.csv`, `output/refined/validate_defined_classes.csv`, `output/refined/validate_class_fates.csv`
