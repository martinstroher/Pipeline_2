...
The ablation, statistical analyses, expert workbooks, rehearsal, and usability-pilot documentation are isolated under `evaluation_study/`. They read frozen production artifacts but are not imported or dispatched by `pipeline.py`. The vNext workbook translates opaque ontology labels through `config/display_text.yaml`, explicitly distinguishes geological-age properties from named time intervals, renders complete non-circular Defined Class sentences, shows a short task banner on every expert-facing sheet, and requires one approved condition-independent reference definition for every Category Correct and Meaning Preservation term. Representation uses 100 terms stratified by Condition-A tier and frequency; Category Correct separately samples 60 A-vs-baseline disagreements across overlap, tier, and frequency, with at least 25 terms per contrast. Every expert receives the same sampled items. Meaning Preservation records semantic retention separately from lean-core appropriateness while retaining its conditional Preferred Outcome and both Taxonomy judgments. Layer 2 reports disagreement-only contrasts with bootstrap intervals and Holm correction, exploratory Holm-corrected Spearman correlations, raw agreement, Fleiss kappa, Gwet AC1/AC2 sensitivity, and item consensus. It makes no runtime LLM call and hashes the approved definition artifact in the workbook manifest. See `evaluation_study/README.md`.
