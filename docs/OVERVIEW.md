...
The validate step also writes lateral-coherence artifacts: `validate_class_fates.csv` (class fate, centrality, cross-axis flags, placement rationale, confidence, non-blocking review flags), `validate_facet_frames.csv` (same-axis/mixed-axis sibling diagnostics), and `validate_disjointness.csv` (candidate same-axis disjointness sets). Relation rows also carry `Relation_Scope`, so corpus-context facts can be kept out of generic OWL class axioms.
