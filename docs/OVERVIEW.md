...
| **7b — Verify** | The artifact is parsed and checked structurally, then HermiT reasons over the configured BFO, GeoCore, and GeoReservoir closure. Any unsatisfiable named class fails verification. RO is attempted separately with a fixed time bound, OOPS! scans the artifact, and executable domain-configured queries report each competency question as FULL, PARTIAL, or NOT ANSWERABLE. |
