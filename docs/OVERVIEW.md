...
| **validate — Critic** | Two focused LLM calls per category, run in sequence. **Call 1 (taxonomy)** sees every term + NLD, the relations on those terms as read-only context, and the allowed target-class list; it emits one verdict per row (`KEEP / REPARENT / DROP_AS_MIXIN / DROP_AS_REDUNDANT / CONVERT_TO_INSTANCE`) and a per-row probe trace (`probe1_genus_ok`, `probe2_bucket`, `probe3_rewrite`) that forces the reasoning before the verdict. **Call 2 (relations)** sees the relation rows, the critic menu (relations flagged `critic_menu: true` in config — 20 for Pre-Salt; mereology offered only as generic `has_part`/`part_of`), previously minted properties, the taxonomy NLDs as context, and a handoff of call 1's decisions; it emits one verdict per relation (`KEEP / DROP / FIX`, where `FIX` may mint a property). After the edits, a deterministic pass re-specialises mereology (continuant vs occurrent parthood) so it stays correct regardless of any FIX. A completeness guard re-asks the model for any input row it forgets, so large categories are not silently under-reviewed. REPARENT is evidence-based (no hard citation floor). CONVERT_TO_INSTANCE rows are recorded in `validate_instances.csv` for emission as `owl:NamedIndividual`; minted properties accumulate in `validate_minted_properties.csv` (provenance `critic_minted`). A full audit log — taxonomy rows carrying the probe trace — is written to `validate_edits.csv`. |
| **7 — Export** | The taxonomy is serialised as an OWL/Turtle file with `rdfs:subClassOf` links, `rdfs:label`, `rdfs:comment` (the NLD), and `owl:imports` for the BFO upper ontology. Critic-minted properties and CONVERT_TO_INSTANCE individuals are emitted from the validate-step outputs, and BFO Quality/Role descendants receive companion `inheres_in some IndependentContinuant` / `realized_in some Process` restrictions. Case-insensitive IRI matching, self-reference guards, and upper→upper triple suppression prevent duplicate/invalid OWL triples. Disjointness conflict detection automatically resolves presalt: classes that inherit from both sides of BFO disjoint pairs, using the term's Category to determine which parent to keep. |
