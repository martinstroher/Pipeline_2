...
| **validate — Critic** | Focused stages run in sequence: taxonomy correctness; evidence-backed class worthiness; deduplication; facet/subsumption/singleton audit; corpus-attested frame completion; relation correctness; and independent relation-scope classification. Deterministic repair preserves the nearest surviving ancestor when parents are dropped. All decisions and uncertainty are written to audit artifacts. |
...
The validate step joins frequency/CQ evidence, then runs focused class-worthiness, dedup, facet/frame, relation-correctness, and relation-scope critics. It writes class-fate/demotion, subsumption, facet, corpus-backed frame-completion, candidate-disjointness, and summary artifacts; uncertain decisions remain non-blocking audit metadata, and only high-confidence generic relations become class axioms.
