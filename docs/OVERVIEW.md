...
| **7 — Export** | The taxonomy is serialized as versioned OWL/Turtle. Unicode labels are transliterated before IRI minting, public comments contain definitions or clean descriptions, configured upper ontologies are imported, and every canonical triple receives a provenance row. A validated static correction manifest can apply reviewed graph transformations without a live LLM call. |
| **7b - Verify** | The artifact is parsed and checked structurally, then HermiT reasons over the configured BFO, GeoCore, and GeoReservoir closure. Any unsatisfiable named class fails verification. RO is attempted separately with a fixed time bound, OOPS! scans the artifact, and executable domain-configured queries report each competency question as FULL, PARTIAL, or NOT ANSWERABLE. Relational CQ reports include both the entity inventory and relation bindings, and any verdict change names its correction cause and checks whether the prior supporting axiom remains. |
...
The thesis ablation, statistics, expert workbooks, offline rehearsal, and usability-pilot methods are isolated from the production pipeline under [`evaluation_study/`](../evaluation_study/README.md). The study reads frozen pipeline artifacts but writes only to `evaluation_study/output/`. GeoPreSalt 0.2 is a separately versioned provisional artifact and does not inherit the expert approval of frozen 0.1. Its correction manifest records machine geologist-critic judgments as reviewable evidence, not as a new human evaluation.
