...
| **validate — Critic** | Focused barriers run in order: taxonomy correctness; evidence-backed class worthiness; within/cross-category term reconciliation (cross-category candidates use in-memory BGE-M3 NLD similarity); facet/subsumption/singleton audit; corpus-attested frame completion with standard NLD/targeted relations; relation correctness; and independent relation-scope classification. Deterministic repair preserves surviving ancestry and all uncertainty is audited. |
...
The validate step keeps generated NLD embeddings separate from the corpus RAG index. Cross-category reconciliation sends each term's top three BGE-M3 NLD neighbors to the LLM with no numeric score cutoff, writes full audit artifacts, and emits only high-confidence generic class relations. Missing frame members without corpus support remain diagnostics rather than LLM-only additions.
