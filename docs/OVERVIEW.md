...
The thesis ablation, statistics, expert workbooks, offline rehearsal, and usability-pilot methods are isolated from the production pipeline under [`evaluation_study/`](../evaluation_study/README.md). The study reads frozen pipeline artifacts but writes only to `evaluation_study/output/`. Representation uses a 100-term tier/frequency sample; Category Review separately samples 60 terms where Condition A differs from at least one baseline, ensuring useful coverage of A-vs-B/C/D comparisons. Expert workbooks use reviewed geological display text, distinguish geological-age properties from named time intervals, render complete definition sentences, use condition-independent reviewed term meanings, and present each sheet with a large plain-language task panel and question-style column names. Relation rows show only the scoped statement; local source excerpts remain private to avoid confusing occurrence with general truth. Every expert receives the same sampled items, including both Taxonomy correctness and usefulness questions. A separate optional `model_changes_review.xlsx` lets a geologist or ontologist review 40 exclusions and rewrites without adding that cognitive burden to the main study. Layer 2 reports bootstrap intervals, multiplicity-corrected disagreement contrasts and exploratory correlations, response distributions, raw agreement, Fleiss kappa, Gwet sensitivity measures, and item consensus. A timing sheet measures actual burden during the human pilot.
