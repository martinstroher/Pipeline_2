...
The thesis ablation, statistics, expert workbooks, offline rehearsal, and usability-pilot methods are isolated from the production pipeline under [`evaluation_study/`](../evaluation_study/README.md). The study reads frozen pipeline artifacts but writes only to `evaluation_study/output/`. Representation uses a 100-term tier/frequency sample; Category Review separately samples 60 terms where Condition A differs from at least one baseline, ensuring useful coverage of A-vs-B/C/D comparisons. Expert workbooks use reviewed geological display text, distinguish geological-age properties from named time intervals, render complete definition sentences, use condition-independent reviewed term meanings, and present each sheet with a large plain-language task panel and question-style column names. Relation rows cover generic, corpus-context, and individual-fact scopes. Removed or Rewritten Terms presents factual before/after states and separately asks whether meaning remains and whether the treatment suits the main Pre-Salt model. Every expert receives the same sampled items, including both Taxonomy correctness and usefulness questions. Layer 2 reports bootstrap intervals, multiplicity-corrected disagreement contrasts and exploratory correlations, response distributions, raw agreement, Fleiss kappa, Gwet sensitivity measures, and item consensus. A timing sheet measures actual burden during the human pilot.
