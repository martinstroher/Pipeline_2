...
The validate step keeps generated NLD embeddings separate from the corpus RAG index, writes reconciliation/fate/demotion/subsumption/facet/completion/disjointness artifacts, and emits only high-confidence generic class relations. Missing frame members without corpus support remain diagnostics rather than LLM-only additions.
