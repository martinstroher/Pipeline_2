...
| **validate — Critic** | Focused barriers run in order: taxonomy correctness; evidence-backed lean-core selection by the existing class-worthiness critic; conflict-safe within/cross-category reconciliation (cross-category candidates use in-memory BGE-M3 NLD similarity); facet/subsumption/singleton audit; corpus-attested frame-completion diagnostics; relation correctness; and independent relation-scope classification. Deterministic repair preserves surviving ancestry and all uncertainty is audited. |
