...
| **5b — Refine** | *(optional, `--refine`)* Cleans encoding/synonym duplicates, then scores each term against 10 competency questions (CQs) in parallel. SPECIALIZATION pairs are passed to Step 6 as parent-child hints. Terms are kept or removed based on how many CQs they contribute to. Produces 4 threshold variants (T=0,1,2,3); Steps 6–7b run independently per threshold to compare ontology sizes. |
...
| **6d — Reclassify** | Deterministic (no LLM) step that uses accepted relations from Step 6b to verify each term's BFO category. The default `refinement` mode is conservative: it only moves a term to a **strict BFO subclass** of its current category, requires at least 2 supporting relations (configurable), and refuses to act when the current category is not in the upper-ontology vocabulary. The legacy `contradiction` mode (selectable via `STEP6D_MODE=contradiction`) is more aggressive — it moves terms whenever the evidence points to a different, more specific category. In both modes, evidence whose implied metatype set spans a BFO disjoint pair (e.g., `MaterialEntity` and `ImmaterialEntity` simultaneously) is logged as a CONTRADICTION and the term is left in place. Actions are recorded as `REFINE`, `RECLASSIFY`, `REPARENT`, or `CONTRADICTION`. |
