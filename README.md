...
domains/                  # Per-domain config + assets, plus shared BFO/RO defaults
  README.md               # Author guide: layout, activation, per-prompt runtime-placeholder contract
  _shared/
    bfo_ro_relations.yaml  # 61 BFO/RO entries + metatype groups + parthood rules (defined once)
  _template/              # Scaffold config loads shared relations; add only domain-specific entries
  presalt/
...
src/
  ...
  utils/
    ontology_config.py    # Cached config loader; optional file-relative relation defaults, env overrides
...
test/                     # Validation suite (run in this order before any production run)
  ...
  test_shared_relation_config.py  # Shared defaults, standalone configs, and new-domain scaffold
  test_shared_relation_regeneration.py  # Offline export matches frozen GeoPreSalt graph
