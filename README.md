...
are downloaded on first use from exact Hugging Face repository revisions
recorded in `src/utils/rag_setup.py`. Both models use standard Transformers
architectures, so mutable repository Python code is not trusted or executed.
Record the downloaded snapshot hashes and Azure deployment version for a study
run. This environment is not a reconstruction of the historical GeoPreSalt
0.1 environment.
...
### Run
The repository includes synthetic `inputs/mock_*.pdf` files to demonstrate the
input location without redistributing papers. Remove them before adding a real
corpus. User-supplied files in `inputs/` are ignored by Git.

Place lawful PDF files in `inputs/`, then:
```bash
python pipeline.py
```
...
| 1 | `term_extractor.py` | `inputs/*.md` | `output/extract_raw.json` |
...
The ablation, statistics, expert workbooks, and rehearsal are isolated from the
production pipeline under [`evaluation_study/`](evaluation_study/README.md).
They read frozen pipeline artifacts and write only to
`evaluation_study/output/`. The public Condition-A file retains its terms,
generated NLDs, and context-use flags but excludes retrieved article passages.
...
output/
  final/                  # Frozen evaluated GeoPreSalt 0.1 + verification receipt
