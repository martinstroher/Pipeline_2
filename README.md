...
| 7 | `owl_exporter.py` | validate CSVs plus optional correction manifest | OWL/Turtle plus complete triple provenance |
| 7b | `emit/verifier.py` | OWL file plus configured upper ontologies and CQ queries | closure-aware HermiT, bounded RO, OOPS!, structure, and CQ reports |
...
To reproduce the versioned GeoPreSalt 0.2 release without live LLM calls:

```bash
JAVA_EXE=/opt/homebrew/opt/openjdk@21/bin/java \
OOPS_URL=http://127.0.0.1:8080/OOPS/rest \
python -m src.modules.emit.release \
  --taxonomy evaluation_study/inputs/approved_run/validate_taxonomy.csv \
  --relations evaluation_study/inputs/approved_run/validate_relations.csv \
  --instances evaluation_study/inputs/approved_run/validate_instances.csv \
  --defined evaluation_study/inputs/approved_run/validate_defined_classes.csv \
  --output-dir output/v0_2 \
  --frozen-artifact output/final/presalt_ontology.ttl \
  --frozen-sha256 5b1c950609b5a8b2cd2e2ade78877f769f950c891eb26b84bba166956bd195bb \
  --baseline-record domains/presalt/corrections/geopresalt_0_1_baseline.json \
  --anti-overfit-terms domains/presalt/anti_overfit_terms.txt
```
...
domains/                  # Per-domain config + assets. Each subfolder is a complete retargetable bundle.
  ...
  presalt/
    ...
    cq_answerability.yaml # Executable post-emission CQ query definitions
    corrections/          # Versioned static correction manifests and baseline evidence
...
src/
  modules/
    ...
    emit/
      ...
      correction_manifest.py # Validated domain-configured graph transformations
      cq_answerability.py    # Executable FULL/PARTIAL/NOT ANSWERABLE CQ checks
      provenance.py          # Stable one-row-per-triple provenance output
      release.py             # Deterministic versioned release and proof gate
      verifier.py            # Step 7b: closure-aware verification (syntax, structure, OOPS!, HermiT, CQ)
  utils/
...
output/
  ...
  v0_2/                   # Versioned provisional 0.2 ontology and proof evidence
